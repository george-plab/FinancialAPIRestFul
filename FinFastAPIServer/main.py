...
async def create_csv(file: UploadFile = File(...), session_id: str = "default"):
    ...
    try:
        ...
        store.store_csv(session_id, file.filename, data, metadata) # pyright: ignore[reportArgumentType]
...
def list_csvs():
    ...
    return {
        "csvs": [
            {
                "session_id": sid,
                "filename": store.get_csv(sid)["filename"], # pyright: ignore[reportOptionalSubscript]
                "uploaded_at": store.get_csv(sid)["uploaded_at"] # pyright: ignore[reportOptionalSubscript]
            }
            for sid in sessions
        ]
    }
...
async def create_analysis(session_id: str, request: AnalysisRequest = None): # pyright: ignore[reportArgumentType]
    """
